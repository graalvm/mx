...
def gate(args):
    ...
    try:
        ...
        with Task('Verify Java Sources in Project', tasks, tags=[Tags.style]) as t:
            if t:
                mx.log(time.strftime('%d %b %Y %H:%M:%S - Ensuring all Java sources files are part of a project...'))
                if mx.command_function('verifysourceinproject')([]) != 0:
                    t.abort('Move or delete the sources files that aren''t part of a project.')
