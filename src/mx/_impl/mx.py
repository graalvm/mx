...
def _discover_repo_suites(start_dir=None):
    ...
    _, enclosing_repo_root = SuiteModel.get_vc(start_dir)
    ...
    for dirpath, dirnames, filenames in os.walk(repo_root):
        ...
        _, suite_repo_root = SuiteModel.get_vc(os.path.abspath(suite_dir))
        suite_repo_root = suite_repo_root if suite_repo_root and exists(suite_repo_root) else repo_root
        discovered.append(_RepoSuiteInfo(_suitename(dirpath), suite_dir, dirpath, join(dirpath, 'suite.py'), suite_repo_root))
...
def _run_command_for_repo_suites(command, discovery):
    if getattr(_opts, 'primary_suite_path', None):
        abort('`-p/--primary-suite-path` cannot be used together with `--all-suites`, `--root-suites`, `--diff-suites`, or `--diff-branch-suites`.')
    if getattr(_opts, 'primary', False) or getattr(_opts, 'specific_suites', []):
        abort('`--primary` and `--suite` cannot be used together with `--all-suites`, `--root-suites`, `--diff-suites`, or `--diff-branch-suites`.')
