...
class JavacCompiler(JavacLikeCompiler):
    ...
    def buildJavacLike(self, jdk, project, jvmArgs, javacArgs, disableApiRestrictions, warningsAsErrors, showTasks):
        javacArgs.append('-Xlint:all,-auxiliaryclass,-try,-processing')
...
def _eclipseinit_project(p, files=None, libFiles=None):
    ...
    if processorPath:
        ...
        if not exists(genDir):
            os.mkdir(genDir)
        out.open('classpathentry', {'kind' : 'src', 'path' : p.source_gen_dir_name()})
...
def _genEclipseBuilder(dotProjectDoc, p, name, mxCommand, refresh=True, refreshFile=None, relevantResources=None, async=False, logToConsole=False, logToFile=False, appendToLogFile=True, xmlIndent='\t', xmlStandalone=None):
    ...
    launchOut.element('stringAttribute', {'key' : 'org.eclipse.ui.externaltools.ATTR_WORKING_DIRECTORY', 'value': _primary_suite.dir})
