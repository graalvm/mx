...
class Suite:
    ...
    def _load_projects(self):
        ...
        jreLibsMap = dict() if suiteDict.get('jrelibraries') is None else suiteDict['jrelibraries']
    ...
    if os.path.isdir(d):
        for f in os.listdir(d):
            if (mxDirName == None and (f == 'mx' or fnmatch.fnmatch(f, 'mx.*'))) or f == mxDirName:
                mxDir = join(d, f)
                if exists(mxDir) and isdir(mxDir) and exists(join(mxDir, 'projects.py')):
                    return mxDir
